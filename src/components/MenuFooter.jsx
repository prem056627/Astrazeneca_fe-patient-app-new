...
function MenuItem({ label, icon: Icon, isActive, onClick }) {
    return (
        <div
            onClick={onClick}
            className={`flex cursor-pointer flex-col w-full items-center gap-1 ${isActive ? 'text-[#BE2BBB]' : 'text-[#595454]'}`}
        >
            <Icon
                className={`h-7 w-7 ${
...
function MenuFooter() {
    ...
    return (
			<nav className="shadow-custom-shadow fixed bottom-0 left-0 flex w-full justify-between gap-4 bg-white px-10 py-6 shadow-lg">
				{(current_page_state === 'hcp_profile' ? hcpMenuItems : menuItems).map(
					(item) => (
						<MenuItem
							key={item.label}
							label={item.label}
